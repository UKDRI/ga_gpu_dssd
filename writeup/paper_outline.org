...
#+CAPTION: The Rule 2 "Iron-Toxic" phenotype is orthogonal to global transcriptomic clusters. (Left) UMAP projection of all cells, with Rule 2 cells colored in red. The lack of a distinct spatial island confirms that this state does not correspond to a single cell type identity. (Right) Prevalence of the Iron-Toxic state across standard Seurat clusters. While some clusters (e.g., 0, 4) show higher susceptibility, the phenotype is distributed across the entire dopaminergic population, indicating a pan-lineage stress response.
