...
We posit that **Subgroup Discovery (SD)** is uniquely suited to solve this problem. Unlike clustering, SD identifies local patterns: logic rules that describe a specific subset of interest without forcing a global partition of the data.

** The Computational Innovation: From Greedy Search to DSSD
Historical applications of Subgroup Discovery in transcriptomics (e.g., Lavrač et al., 2004; Pieters et al., 2010) typically focused on microarray data, where "instances" were patients. The goal was patient stratification. In the single-cell era, we treat **cells as instances**, allowing us to map the landscape of cellular states.

However, applying SD to high-dimensional single-cell data presents a massive search space ($2^{1000}$ combinations). Standard "Top-$k$" algorithms are greedy: they tend to return highly redundant variations of the single strongest signal—a phenomenon known as the **"Table Mountain" effect** (van Leeuwen & Knobbe, 2012).

To overcome this, we implement **Diverse Subgroup Set Discovery (DSSD)**. Instead of optimizing individual rules, we optimize a *set* of rules simultaneously. By penalizing overlap between rules in the fitness function, we force the algorithm to ignore redundant variations and discover distinct, orthogonal biological mechanisms.
